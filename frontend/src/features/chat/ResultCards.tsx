    ...
    <div className="grid grid-cols-1 gap-2">
      {visible.map((card) =>
        card.productType === 'hotel' ? (
          <HotelCard key={card.product.id} product={card.product} criteria={hotelCriteria} compact />
        ) : (
          <FlightCard key={card.product.id} product={card.product} criteria={flightCriteria} compact />
        ),
      )}
